...
namespace seal
{
    ...
    DecryptorT::DecryptorT(const EncryptionParameters &parms, const BigPoly &secret_key) :
    ...
    void DecryptorT::decryptSPU(BigPolyArray &encrypted, BigPoly &destination)
    {
    ...
    void DecryptorT::decryptMU(const BigPolyArray &encrypted, BigPoly &destination, BigPoly &cpSPU)
    {
        // Extract encryption parameters.
        // Remark: poly_modulus_ has enlarged coefficient size set in constructor
        int coeff_count = poly_modulus_.coeff_count();
